...
REST_ACTION_FIELD = 'set_field'
REST_ACTION_METER = 'meter'
REST_ACTION_OUTPUT = 'output'
REST_ACTION_VLAN_PUSH = 'push_vlan'
REST_ACTION_VLAN_POP = 'pop_vlan'
...
class RestQoSAPI(app_manager.RyuApp):
    ...
    def stats_reply_handler(self, ev):
        ...
        elif dp.ofproto.OFP_VERSION == ofproto_v1_4.OFP_VERSION:
            flags = dp.ofproto.OFPMPF_REPLY_MORE
        if msg.flags & flags:
            return
...
class QoSController(ControllerBase):
    ...
    def regist_ofs(dp, CONF):
        ...
        try:
            f_ofs = QoS(dp, CONF)
            # Do not register goto table flow by default
    ...
    def set_ovsdb_addr(dpid, value):
        ...
        QoSController._LOGGER.info('set_ovsdb_addr ofs %s', ofs)
        if ofs is not None:
...
class QoS(object):
    ...
    def set_ovsdb_addr(self, dpid, ovsdb_addr):
        ...
        QoSController._LOGGER.info("dpid %s, ovsdb_addr %s", dpid, ovsdb_addr)
    ...
    def set_queue(self, rest, vlan_id):
        ...
        port = rest.get(REST_PORT_NAME, None)
        vif_ports = self.ovs_bridge.get_port_name_list()

        map_port_names = {}
        ex_vif_ports = self.ovs_bridge.get_port_name_list()
        for port_name in ex_vif_ports:
            port_no = self.ovs_bridge.get_ofport(port_name)
            map_port_names[port_no] = port_name

        if port is not None:
            port_no = int(port)
            if port_no not in map_port_names:
                raise ValueError('%s port is not exists' % port)
            port_name = map_port_names[port_no]
            vif_ports = [ str(port_name) ]
        ...
        QoSController._LOGGER.info('queue rest %s', rest)
        ...
        for queue in queues:
            ...
            QoSController._LOGGER.info('max_rate %s min_rate %s', max_rate, min_rate)
        ...
        QoSController._LOGGER.info('queue_type %s, parent_max_rate %s, queue_config %s', queue_type, parent_max_rate, queue_config)
        QoSController._LOGGER.info('vif_ports %s', vif_ports)
        ...
        for port_name in vif_ports:
            try:
                ...
                QoSController._LOGGER.info('error port-name %s, msg %s', port_name, msg)
                raise ValueError(msg)
            self.queue_list[port_name] = queue_list

        QoSController._LOGGER.info('queue_list %s', self.queue_list)

        msg = {'result': 'success',
               'details': queue_list}

        QoSController._LOGGER.info('msg ok %s', msg)
        return REST_COMMAND_RESULT, msg
    ...
    def delete_queue(self, rest, vlan_id):
        ...
        QoSController._LOGGER.info('queue_list %s', self.queue_list)
        QoSController._LOGGER.info('port %s type_port %s map_port_names %s',
                                   port, type(port), map_port_names)
        ...
        if port is not None:
            ...
            if port_no not in map_port_names:
                raise ValueError('%s port is not exists' % port)
            port_name = map_port_names[port_no]
            conf_vif_ports = [ str(port_name) ]

        vif_ports = self.ovs_bridge.get_external_ports()

        for port in vif_ports:
            QoSController._LOGGER.info('port.port_name %s in conf_vif_ports %s',
                                       port.port_name, conf_vif_ports)
            if port.port_name in conf_vif_ports:
                try:
                    QoSController._LOGGER.info('del-qos port.port_name %s', port.port_name)

                    self.ovs_bridge.del_qos(port.port_name)
                    del self.queue_list[port.port_name]
                    msg = 'success'
                except Exception as msg:
                    QoSController._LOGGER.info('error port-name %s, msg %s', port.port_name, msg)
                    raise ValueError(msg)
    ...
    def _set_qos(self, cookie, rest, waiters, vlan_id):
        ...
        QoSController._LOGGER.info('match %s', match)

        actions = []
        action = rest.get(REST_ACTION, None)
        if action is not None:
            if REST_ACTION_VLAN_PUSH in action:
            ...
            if REST_ACTION_FIELD in action:
                act = action[REST_ACTION_FIELD]
                actions.append({'type': 'SET_FIELD',
                                'field': act['field'],
                                'value': act['value']})
            ...
            if REST_ACTION_VLAN_POP in action:
        ...
        QoSController._LOGGER.info('actions %s', actions)

        # NO NEED TO GOTO_TABLE SINCE ITS OUTPUT SET
    ...
    def _set_meter(self, rest, waiters):
        ...
        QoSController._LOGGER.info('set_meter %s', rest)
        ...
        msg = {'result': 'success',
               ...
               rest[REST_METER_ID]}
        # QoSController._LOGGER.info('msg %s', msg)
...
class Match(object):
    ...
    _CONVERT = {REST_DL_TYPE:
                {REST_DL_TYPE_ARP: ether.ETH_TYPE_ARP,
                 ...
                 REST_DL_TYPE_VLAN: ether.ETH_TYPE_8021Q},
                REST_NW_PROTO:
    ...
    def to_openflow(rest):
        ...
        # if REST_DL_VLAN in rest:
        #     match[REST_DL_VLAN] = rest[REST_DL_VLAN]
        ...
        if dl_type is not None:
            ...
            elif dl_type == REST_DL_TYPE_VLAN:
                pass
            else:
                pass
    ...
    def to_rest(flow):
        if REST_ACTION in flow:
            ...
            for act in flow[REST_ACTION]:
                ...
                if output_value:
                    actions.append({REST_ACTION_OUTPUT: output_value.group(1)})
